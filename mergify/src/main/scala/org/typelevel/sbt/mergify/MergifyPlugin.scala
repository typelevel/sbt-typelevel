...
object MergifyPlugin extends AutoPlugin {
  ...
  private lazy val generateMergifyContents = Def.task {
    compileMergify(mergifyPrRules.value.toList)
  }

  private def compileMergify(prRules: List[MergifyPrRule]): String = {
    s"""|# This file was automatically generated by sbt-typelevel-mergify using the
        ...
        |pullRequestRules:${compileList(prRules.map(compilePrRule), 2, 0)}
        |""".stripMargin
  }

  private def compilePrRule(rule: MergifyPrRule): String = {
    s"""|name: ${rule.name}
        |conditions:${compileList(rule.conditions.map(compileCondition), 2, 0)}
        |actions:${compileList(rule.actions.map(compileAction), 2, 0)}
        |""".stripMargin
  }

  private def compileCondition(condition: MergifyCondition): String = {
    import MergifyCondition._
    condition match {
      case Custom(condition) => condition
      case And(conditions) => s"and:${compileList(conditions.map(compileCondition(_)), 2, 0)}"
      case Or(conditions) => s"or:${compileList(conditions.map(compileCondition(_)), 2, 0)}"
      case _ => sys.error("should not happen")
    }
  }

  private def compileAction(action: MergifyAction): String = {
    import MergifyAction._
    action match {
      case Merge(method, rebaseFallback, commitMessageTemplate) =>
        method.fold("")(m => s"method: ${m}\n") +
          rebaseFallback.fold("")(rf => s"rebase_fallback: ${rf}\n") +
          commitMessageTemplate.fold("")(cmt => s"commit_message_template: ${cmt}\n")
      case Label(add, remove, removeAll) =>
        Some(add).filter(_.nonEmpty).fold("")(a => s"add:${compileList(a, 2)}\n") +
