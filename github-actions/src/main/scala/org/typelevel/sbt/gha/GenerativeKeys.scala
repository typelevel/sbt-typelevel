...
trait GenerativeKeys {
  ...
  lazy val githubWorkflowBuildConcurrency = settingKey[Option[JobConcurrency]](
    s"Use concurrency to ensure that only a single build job within the same concurrency group will run at a time. (default: $${{ matrix.os }}-$${{ matrix.java }}-$${{ matrix.scala }}-$${{ github.ref }})")
