...
- `githubWorkflowBuildConcurrency` : `Option[JobConcurrency]` - Use concurrency to ensure that only a single build job within the same concurrency group will run at a time. Defaults to `ci-${{ matrix.os }}-${{ matrix.java }}-${{ matrix.scala }}-${{ github.ref }}`.
